...
classifications_file <- "/Users/ebar/Dropbox/R/Zooniverse_work/UseThisCode/Testing_annotations.csv"
```
####Examine data
```{r}
...
Try to crack this function and see what each step is doing
...
```{r}
debug(run_json_parsing)
run_json_parsing(jdata)
```
...
#reduced the data frame to 5 from 14 variables

flat_to_task %>% as.tbl_json(json.column = "annotations") 

#made annotations column into a json table?
          
flat_to_task %>%gather_array(column.name = "task_index")


# really important for joining later
          spread_values(flat_to_task$annotations, task = jstring("task"), task_label = jstring("task_label"), value = jstring("value"))
