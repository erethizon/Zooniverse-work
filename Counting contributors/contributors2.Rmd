...
```
Now let's add the date of the first day of the week, with week beginning on a Sunday.  Here are some ideas

```{r}
DF3$startDay<-strptime(paste(year(DF3$created_at),DF3$week,7), format = "%Y %U %u")
```
And now breakdown by week but keep start date
```{r}
UserByWeek<-DF3 %>% group_by(as.factor(startDay),userFactor) %>% summarise(
     NumClass = length(classification_id),
)

```
Looks good.  
Now try to filter by max user each week
```{r}
mainClassifier<-UserByWeek %>% filter()
```
