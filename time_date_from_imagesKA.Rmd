...
Kate's attempt to order photos based on event number in spreadsheet for Zooniverse
```{r}
library(data.table)
MyVector<-as.vector(paste0(DFnames[1],"$Result"))
l<-length(Meta1$Result)
Group1<- 1

Image1<- Meta1[match(unique(Meta1$Result), Meta1$Result),]
Image2<- setDT(Meta1)[, .SD[2L], by = Result]
Image3<- setDT(Meta1)[, .SD[3L], by = Result]
